...
{
    assert(bound >= BOUND_UPPER && bound <= BOUND_EXACT);
    ...
    if (auto key32 = static_cast<int32_t>(key); key32 != tte->key || depth + 4 >= tte->depth || bound == BOUND_EXACT || Age(tte))
        tte->key   = key32,
        tte->score = score,
        tte->eval  = eval,
        tte->depth = depth,
...
static void* ThreadClearTT(Thread* thread) 
{
