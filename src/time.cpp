...
void InitTimeManagement() {
    ...
    if (!Limits.movestogo) {
        Limits.optimalUsage = static_cast<int>(MIN(0.022 * timeLeft, 0.2 * Limits.time));
    } else {
        // X moves in Y time
        Limits.optimalUsage = static_cast<int>(MIN(timeLeft * 0.7 / mtg, 0.8 * Limits.time));
    }

    Limits.maxUsage = static_cast<int>(MIN(5 * Limits.optimalUsage, 0.8 * Limits.time));
}
